...
What is a K-Pass Test?
    ...
    A question PASSES a pass if it scores >= 0.90 on that pass.
    Overall pass rate = passes passed / (questions x K)

Output: k_pass_results_e.txt (saved to same folder as this script)

Usage:
    python K_pass_test_e.py --disprot Data/DisProt_ProteinData.json --qa Data/QA_Dataset.json
    python K_pass_test_e.py --demo
"""
...
def write_results(results, stats):
    ...
    lines = []
    lines.append("=" * 70)
    lines.append(f"  BMEN-499 AlphaFold -- K-Pass Test E (K={K}, Threshold=0.90 Maximum Strict): LLM Judge 1")
    ...
    lines.append("")
    lines.append("  Pass threshold: score >= 0.90 to PASS a pass.")
    ...
    for p in PASSES:
        ...
        prate  = sum(1 for s in pscores if s >= 0.90) / len(pscores)
    ...
    out_path   = os.path.join(script_dir, "k_pass_results_e.txt")
...
def main():
    parser = argparse.ArgumentParser(
        description=f"K-Pass Test E (K={K}, Threshold=0.90 Maximum Strict): LLM Judge 1 predictions vs ground truth"
    )
